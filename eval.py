...
def analyze_reranking_improvement(before, after):
	out = {}
	for key, rank_before in before.items():
...
def eval_short(res_fname,
                  format="trec",
                  th=10, 
                  verbose=False,
                  reranking_th=0.0,
                  ignore_noanswer=False):
	ir, svm, conf_matrix = read_res_pred_files(res_fname, res_fname, format, verbose, 
	...
	#print("MAP   : %6.2f" %(map_se*100))
	#print("MRR   : %6.2f" %(mrr_se))
	#print("Acc@1 : %6.2f"%(acc_se[0]))
    
	return map_se, mrr_se, acc_se[0], ap
...
def eval_reranker(res_fname="svm.test.res", pred_fname="svm.train.pred", 
	...
	print()
	print("ALL SCORES:\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%5.4f" %(map_svm, avg_acc1_svm, mrr_svm, p, r, f1, acc))
